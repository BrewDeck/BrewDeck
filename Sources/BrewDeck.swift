...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance optimized stored properties
    ...
    /// Recalculates performance-sensitive derived properties and stores them.
    /// This avoids redundant UserDefaults lookups and string parsing in the render loop.
    mutating func updateDerivedState() {
        ...
        // 2. Calculate rating (cached from UserDefaults or hash fallback)
...
class BrewManager: ObservableObject {
    ...
    func updatePackageRating(id: String, rating: Double) {
        ...
        logDebug("BrewManager: Saved custom rating \(rating) for \(id). Updating memory model.")

        // Update the main packages list
        if let index = self.packages.firstIndex(where: { $0.id == id }) {
            self.packages[index].updateDerivedState()
        }

        // Also update recommendedPackages to keep UI in sync
        if let index = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            self.recommendedPackages[index].updateDerivedState()
        }

        // Force an objectWillChange if needed, though @Published usually handles it
