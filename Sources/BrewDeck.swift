...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Bolt: Performance optimization - converted to stored properties to avoid expensive re-calculation
    // during list rendering and filtering.
...
class BrewManager: ObservableObject {
    ...
    // Bolt: Performance optimization - added centralized rating update to ensure memory models
    // and UserDefaults stay in sync for optimized UI feedback.
    func updatePackageRating(id: String, rating: Int) {
        if let index = packages.firstIndex(where: { $0.id == id }) {
            ...
            logDebug("Updated rating for \(id) to \(rating) and refreshed derived state.")
        }
    }
