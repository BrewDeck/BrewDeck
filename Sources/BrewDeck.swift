...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Stored properties for derived state to avoid expensive re-calculation
    ...
    mutating func updateDerivedState() {
        ...
        // Calculate rating (User rating if available, otherwise deterministic hash-based rating)
        self.rating = {
        ...
        // Calculate deterministic rating count
        self.ratingCount = {
            let hash = abs(id.hashValue)
...
class BrewManager: ObservableObject {
    ...
    func updatePackageRating(id: String, rating: Double) {
        ...
        // Update in-memory state for primary package list
        if let index = packages.firstIndex(where: { $0.id == id }) {
            packages[index].updateDerivedState()
        }

        // Update in-memory state for recommended packages carousel
