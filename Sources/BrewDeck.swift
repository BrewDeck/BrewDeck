...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance optimized stored properties for frequent UI access
    ...
    /// Recalculates stored properties to avoid expensive re-computation in UI loops
    mutating func updateDerivedState() {
        ...
        // Update rating from persistent storage or generate stable hash-based fallback
        self.rating = {
        ...
        // Update rating count display string
        self.ratingCount = {
            let hash = abs(id.hashValue)
...
class BrewManager: ObservableObject {
    ...
    /// Centralized rating update to keep UserDefaults and in-memory models in sync
    func updatePackageRating(id: String, rating: Int) {
        ...
        logDebug("Updated rating for \(id) to \(rating). State refreshed.")
    }
