...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    mutating func updateDerivedState() {
        ...
        // rating calculation (memoized in stored property)
...
class BrewManager: ObservableObject {
    ...
    func updatePackageRating(id: String, rating: Double) {
        ...
        if let idx = self.packages.firstIndex(where: { $0.id == id }) {
            self.packages[idx].updateDerivedState()
        }

        // Also update recommendedPackages if it contains this package
        if let recIdx = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            self.recommendedPackages[recIdx].updateDerivedState()
        }

        self.objectWillChange.send()
        logDebug("BrewManager: Updated rating to \(rating) for \(id) and refreshed derived state.")
    }
