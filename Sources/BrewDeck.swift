...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    /// Recalculates stored properties to avoid expensive repeated computations in UI render cycles.
    mutating func updateDerivedState() {
        // 1. Calculate update status (supports Homebrew revisions like _1)
        ...
        // 2. Calculate rating (prioritizes local UserDefaults override)
        ...
        // 3. Calculate rating count string
        let hash = abs(id.hashValue)
...
class BrewManager: ObservableObject {
    ...
    /// Updates the rating for a package, persisting to UserDefaults and refreshing in-memory models.
