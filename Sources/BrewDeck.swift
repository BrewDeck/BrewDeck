...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance optimizations: pre-calculate derived values
    var hasUpdate: Bool = false
    var rating: Double = 4.5
    var ratingCount: String = "12,000"
...
class BrewManager: ObservableObject {
    ...
    func refreshRecommendations(force: Bool = false) {
        ...
        if !force && (now - lastTime < 86400) {
            if let storedIds = UserDefaults.standard.stringArray(forKey: "recommended_package_ids") {
                ...
                let cached = self.packages.filter { storedSet.contains($0.id) }
                if !cached.isEmpty {
    ...
    func updatePackageRating(id: String, rating: Int) {
        ...
        logDebug("Persisted centralized user rating \(rating) for package: \(id)")

        // Update both primary list and recommendations for consistent UI
