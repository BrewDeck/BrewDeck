...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance Optimized: Stored properties for derived state to avoid redundant calculations in list renders
    ...
    mutating func updateDerivedState() {
        ...
        // Update rating (Performance: Fetching from UserDefaults only once here instead of during every UI render)
...
class BrewManager: ObservableObject {
    ...
    func updatePackageRating(id: String, rating: Double) {
        ...
        if let index = self.packages.firstIndex(where: { $0.id == id }) {
            var updated = self.packages[index]
            UserDefaults.standard.setValue(rating, forKey: "custom_rating_\(id)")
            updated.updateDerivedState()
            self.packages[index] = updated
        }

        // Also update recommendedPackages to keep UI in sync
        if let recIndex = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            var updated = self.recommendedPackages[recIndex]
            updated.updateDerivedState()
            self.recommendedPackages[recIndex] = updated
        }

        logDebug("Centrally updated rating for \(id) to \(rating) and synced with storage.")
    }
