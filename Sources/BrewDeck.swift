...
class BrewManager: ObservableObject {
    ...
    func refreshRecommendations(force: Bool = false) {
        ...
        // Custom matching rules (from RecommendedPackagesCarousel)
        var ruleBasedIds: [String] = []
        if installed.contains(where: { $0.id.contains("code") || $0.id == "iterm2" || $0.id == "docker" }) {
            ruleBasedIds.append(contentsOf: ["iterm2", "docker", "postman", "visual-studio-code"])
        }
        if installed.contains(where: { $0.id == "git" }) {
            ruleBasedIds.append(contentsOf: ["gh", "lazygit"])
        }
        // Premium default utilities
        ruleBasedIds.append(contentsOf: ["rectangle", "alfred", "vlc", "stats", "appcleaner", "cyberduck", "handbrake"])

        // Filter out already installed or duplicated rule-based IDs
        var finalRuleBasedIds: [String] = []
        for id in ruleBasedIds {
            if !installedIds.contains(id) && !finalRuleBasedIds.contains(id) {
                finalRuleBasedIds.append(id)
            }
        }

        // Gather packages for rule-based IDs
        let ruleBasedPackages = self.packages.filter { finalRuleBasedIds.contains($0.id) }

        // Filter out already installed packages for scoring
        let candidates = self.packages.filter { $0.installedVersion == nil && !finalRuleBasedIds.contains($0.id) }
        ...
        // Sort descending and take top remaining to fill up to 10 recommendations
        scored.sort { $0.score > $1.score }
        let topScored = scored.prefix(max(0, 10 - ruleBasedPackages.count)).map { $0.pkg }

        let top = ruleBasedPackages + topScored
