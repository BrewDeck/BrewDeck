...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance: Stored properties for derived state to avoid redundant calculations and UserDefaults lookups in render cycles.
    ...
    /// Centralized update for performance-sensitive derived properties.
    mutating func updateDerivedState() {
        ...
        // Cache rating from UserDefaults or fallback hash-based score
        self.rating = {
    ...
    /// Internal method for manager to update rating without full re-calculation
    mutating func setRating(_ newRating: Double) {
...
class BrewManager: ObservableObject {
    ...
    /// Centralized rating update for synchronization between memory and persistence.
    func updatePackageRating(id: String, rating: Int) {
        let doubleRating = Double(rating)
        UserDefaults.standard.setValue(doubleRating, forKey: "custom_rating_\(id)")

        // Update in-memory collections for immediate UI feedback without re-computation
        if let idx = self.packages.firstIndex(where: { $0.id == id }) {
            self.packages[idx].setRating(doubleRating)
        }
        if let idx = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            self.recommendedPackages[idx].setRating(doubleRating)
        }

        logDebug("Manager updated rating for \(id) to \(rating)")
    }
    ...
    func refreshRecommendations(force: Bool = false) {
        ...
        if !force && (now - lastTime < 86400) {
            if let storedIds = UserDefaults.standard.stringArray(forKey: "recommended_package_ids") {
                ...
                let cached = self.packages.filter { idSet.contains($0.id) }
                if !cached.isEmpty {
