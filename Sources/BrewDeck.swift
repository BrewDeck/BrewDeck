...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Bolt: Optimized stored properties for high-frequency UI paths (list rendering, filtering, grouping)
    // to avoid redundant O(N) string parsing and UserDefaults lookups in render loops.
    ...
    /// Bolt: Refreshes derived performance properties from raw metadata.
    mutating func updateDerivedState() {
        // rating - cache from UserDefaults or calculate stable hash-based fallback
        ...
        // ratingCount - stable hash-based calculation
        let hash = abs(id.hashValue)
        ...
        // hasUpdate - avoid string parsing in render path
...
class BrewManager: ObservableObject {
    ...
    /// Bolt: Updates package rating and refreshes in-memory stored performance properties.
