...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Stored properties for performance optimization (cached derived state)
...
class BrewManager: ObservableObject {
    ...
    func updatePackageRating(id: String, rating: Double) {
        ...
        // Update the package in both lists to ensure UI consistency
        ...
        if let idx = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            self.recommendedPackages[idx].updateDerivedState()
        }
    }
