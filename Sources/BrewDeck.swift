...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance: Stored properties for expensive calculations
    ...
    enum CodingKeys: String, CodingKey {
        ...
        // Exclude derived performance properties from JSON schema
    }
    ...
    /// Centralized update for derived/cached properties to optimize UI rendering performance.
    mutating func updateDerivedState() {
        ...
        // Update rating (O(1) lookup after calculation)
...
class BrewManager: ObservableObject {
    ...
    /// Updates a package's rating and ensures all collections (main list and recommendations) are synchronized.
    func updatePackageRating(id: String, rating: Double) {
        UserDefaults.standard.set(rating, forKey: "custom_rating_\(id)")

        // Update main packages list
        if let index = self.packages.firstIndex(where: { $0.id == id }) {
            self.packages[index].updateDerivedState()
        }

        // Update recommended packages list
        if let recIndex = self.recommendedPackages.firstIndex(where: { $0.id == id }) {
            self.recommendedPackages[recIndex].updateDerivedState()
        }

        logDebug("Synchronized rating update (\(rating)) for package: \(id)")
    }
    ...
    private func mergeWithLocalPackages(localList: [BrewPackage]) {
        ...
        self.packages = self.packages.map { pkg in
            ...
            updated.updateDerivedState() // Refresh hasUpdate and rating
            return updated
        }
                ...
                DispatchQueue.main.async {
                    self.packages = self.packages.map { pkg in
                        if let latestVer = outdatedMap[pkg.id] {
                            ...
                            updated.updateDerivedState() // Refresh hasUpdate
                            return updated
                        }
                        return pkg
                    }
