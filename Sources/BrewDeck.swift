...
struct BrewPackage: Identifiable, Codable, Equatable {
    ...
    // Performance optimized stored properties to avoid redundant calculations/lookups
    ...
    /// Refreshes performance-sensitive metadata. Should be called after version or installation state changes.
    mutating func updateDerivedState() {
        ...
        // 2. Resolve rating (stored lookup)
        ...
        // 3. Resolve rating count (deterministic fallback)
        let hash = abs(id.hashValue)
...
class BrewManager: ObservableObject {
    ...
    /// Updates a package rating both in UserDefaults and in the memory store.
