...
## 2024-05-28 - [Caching Derived Package Metadata]
**Learning:** Synchronous disk/cache I/O like `UserDefaults` reads inside a SwiftUI `body` or a list's computed property is a critical performance bottleneck. Moving expensive logic (string parsing, version comparison, and metadata lookups) from computed properties to stored properties that are updated only when source data changes significantly improves UI responsiveness.
**Action:** Use stored properties and a centralized `updateDerivedState()` method in core models that are frequently accessed during rendering to minimize redundant computations and I/O.
